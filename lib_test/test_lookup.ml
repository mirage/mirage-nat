...
let show_table_entry (proto, left, right, translate, translate_right) = Printf.sprintf
    ...
    (Ipaddr.to_string (fst translate_right)) (snd translate_right)
...
let default_table () =
  ...
  let t = or_error (insert t 6 interior_v4 exterior_v4 translate_v4 exterior_v4) in
  let t = or_error (insert t 17 interior_v6 exterior_v6 translate_v6 exterior_v4) in
  t
    ...
    let for_all h entries fn =
      List.fold_left (
        fun h (protocol, left, right, translate) -> (
          match fn h protocol left right translate right with
          | None ->
            let str = show_table_entry (protocol, left, right, translate, right) in
