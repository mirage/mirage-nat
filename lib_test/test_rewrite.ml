...
let test_nat_ipv4 proto () =
  ...
  let table = Constructors.make_table_with_nat packet_private ~xl ~xlport_gen:(fun () -> Some xlport) in
...
let test_add_redirect_valid_pkt () =
  ...
  let table =
    Constructors.make_table_with_redirect packet
      ~internal_xl:nat_internal_ip ~internal_xl_port_gen:(fun () -> Some nat_internal_port)
  ...
  let r =
    Rewriter.add  table packet
      (Ipaddr.V4.of_string_exn "8.8.8.8") (fun () -> Some nat_internal_port)
      (`Redirect (internal_client, internal_client_port))
  in
...
let test_add_nat_valid_pkt () =
  ...
  let table = Constructors.make_table_with_nat packet ~xl ~xlport_gen:(fun () -> Some xlport) in
  ...
  let r = Rewriter.add table packet xl (fun () -> Some xlport) `NAT in
...
let test_add_nat_broadcast () =
  ...
  let t = Rewriter.empty ~tcp_size:10 ~udp_size:10 ~icmp_size:10 in
  let r = add t broadcast xl (fun () -> Some xlport) `NAT in
      ...
      | Error `Untranslated ->
        ...
        let r =
          ...
          | 0 ->
            ...
            Rewriter.add  t packet fixed_external_ip
              (fun () -> Some (random_port ()))
              (`Redirect (fixed_internal_ip, random_port ()))
          | _ ->
            Printf.printf "adding a NAT rule\n%!";
            Rewriter.add t packet fixed_external_ip
              (fun () -> Some (random_port ())) `NAT
        in
        match r with
        | Error `Cannot_NAT ->
...
let test_ping () =
  ...
  (* Add rule *)
  let r = Rewriter.add t packet (Ipaddr.V4.of_string_exn "82.1.1.8") (fun () -> Some 81) `NAT in
...
let test_ping_icmp_error () =
  ...
  (* Add rule *)
  let r = Rewriter.add t packet nat_ip (fun () -> Some 81) `NAT in
  Alcotest.check add_result "Add ICMP rule" (Ok ()) r;
  (* Translate outgoing request *)
  let expected = Constructors.make_icmp ~src:"82.1.1.8" ~dst:"8.8.8.8" (`Echo_request (81, 9, payload)) ~ttl:63 in
  let r = Rewriter.translate t packet in
